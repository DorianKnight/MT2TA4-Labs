...
//*********************Testing I2C EEPROM------------------
 ...
 //test realtime clock	
    BSP_LCD_Clear(LCD_COLOR_WHITE);
		...
		HAL_RTC_GetTime(&RTCHandle, &read_RTC_TimeStruct, RTC_FORMAT_BIN);
		LCD_DisplayString(5,0, (uint8_t *) "HH:MM:SS");
	
		LCD_DisplayInt(6,0,read_RTC_TimeStruct.Hours);
		LCD_DisplayInt(6,3,read_RTC_TimeStruct.Minutes);
		LCD_DisplayInt(6,6,read_RTC_TimeStruct.Seconds);
		...
		HAL_RTC_GetTime(&RTCHandle, &read_RTC_TimeStruct, RTC_FORMAT_BIN);
		LCD_DisplayString(7,0, (uint8_t *) "HH:MM:SS");
...
while (1)
  {
		...
		switch (currentState)
		{
			case 0:
				BSP_LCD_Clear(LCD_COLOR_WHITE);
		HAL_Delay(10);
				//Idle State - Display the current time in HH:MM:SS
			...
			BSP_LED_Toggle(LED3);
				...
				//Display to screen
				LCD_DisplayString(0,0, (uint8_t *) "Current time");
		...
		/*BSP_LCD_Clear(LCD_COLOR_WHITE);
		HAL_Delay(10);  
		LCD_DisplayString(1,0,(uint8_t *)"Test");
		HAL_Delay(1000); */
  } //end of while
...
void HAL_RTC_AlarmAEventCallback(RTC_HandleTypeDef *hrtc)
{
	...
	/*BSP_LCD_Clear(LCD_COLOR_WHITE);
	HAL_Delay(10);
	LCD_DisplayString(1,2,(uint8_t *)"TEST???");*/
